...
This script visualizes the metrics and feature importances generated by the 
Random Forest training step (02_train_rf.py).

Features:
- Generates bar plots for model skill metrics (R² and Pearson) across all targets.
- Produces feature importance bar charts (split into hydrological and increment targets).
- Supports comparative plots between temporal and spatial cross-validation.
- Figures are automatically saved in the configured output directory (default: results/figures).

Usage:
    python 03_plot_results.py [--mode MODE] [--dataset DATASET] [--cv CV_MODE] [--mini-test]
"""
...
def plot_results():
    ...
    output_figures = paths.get("output_figures_dir", "results/figures")
