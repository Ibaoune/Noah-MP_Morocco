...
! ================================================
  ...
  elseif(steptype==2)then
     do ic=1,nseqall
        !if flowtype is hybrid or urban, and flowmap is kinematic or
        !urban-kinematic, skip adaptive time step
        if((flowtype==0.or.flowtype==4).and. &
             (flowmap(ic)==1.or.flowmap(ic)==5))then
           dta(ic)=dt
           nt(ic)=1
        else
           call HYMAP3_dynstp(dt,nxtdst(ic),rivdph(ic),grv,cadp,dta(ic),&
                nt(ic))
           if(dta(ic)>dt)then
              dta(ic)=dt
              nt(ic)=1
...
!find the maximum of i and minimum dt across all processors
  ...
  do it=1,maxi
     ...
     call HYMAP3_model_core(n,it,mis,nseqall,nz,time,        &
