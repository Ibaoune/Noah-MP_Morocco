...
!BOP
! !ROUTINE: HYMAP3_routing_writerst
! \label{HYMAP3_routing_writerst}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine HYMAP3_routing_writerst(n)

!
! !DESCRIPTION:
!  This routine writes restart files for HYMAP3. The restart files
  ...
  if(alarmCheck.or.(LIS_rc%endtime ==1)) then
     if(LIS_masterproc) then
        ...
        call LIS_create_restart_filename(n,filename,&
             ...
             wformat="netcdf")
        write(LIS_logunit,*) '[INFO] Writing routing restart ', &
             trim(filename)

#if (defined USE_NETCDF4)
        status = nf90_create(path=filename, cmode=nf90_hdf5, ncid = ftn)
        call LIS_verify(status, &
             "Error in nf90_open in HYMAP3_routing_writerst")
#endif
#if (defined USE_NETCDF3)
        status = nf90_create(Path = filename, cmode = nf90_clobber, &
             ncid = ftn)
        call LIS_verify(status, &
             "Error in nf90_open in HYMAP3_routing_writerst")
#endif
     endif
     call HYMAP3_dump_restart(n, ftn)

     if (LIS_masterproc) then
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
        status = nf90_close(ftn)
        call LIS_verify(status, &
             "Error in nf90_close in HYMAP3_routing_writerst")
#endif
     endif

  endif

end subroutine HYMAP3_routing_writerst

!BOP
!
! !ROUTINE: HYMAP3_dump_restart
! \label{HYMAP3_dump_restart}
!
! !REVISION HISTORY:
!
! !INTERFACE:
...
!EOP
  ...
  call HYMAP3_writeGlobalHeader_restart(ftn, n, &
       "HYMAP3", &
       dimID,dimID_bif)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, rivsto_ID, "RIVSTO", &
       "river storage", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, fldsto_ID, "FLDSTO", &
       "flood storage", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, rnfsto_ID, "RNFSTO", &
       "runoff storage", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, bsfsto_ID, "BSFSTO", &
       "baseflow storage", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, rivout_pre_ID, &
       "RIVOUT_PRE", &
       "rivoutpre", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, rivdph_pre_ID, &
       "RIVDPH_PRE", &
       "river depth_pre", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, fldout_pre_ID, &
       "FLDOUT_PRE", &
       "flood discharge pre", &
       "-", 1, -99999.0, 99999.0)
  call HYMAP3_writeHeader_restart(ftn, n, dimID, flddph_pre_ID, &
       "FLDDPH_PRE", &
       "flood depth pre", &
       "-", 1, -99999.0, 99999.0)
!ag (27Apr2020)
  if(HYMAP3_routing_struc(n)%flowtype==4)then
     call HYMAP3_writeHeader_restart(ftn, n, dimID, drsto_ID, "DRSTO", &
          "urban drainage storage", &
          "-", 1, -99999.0, 99999.0)
     call HYMAP3_writeHeader_restart(ftn, n, dimID, drout_ID, "DROUT", &
  ...
  if(HYMAP3_routing_struc(n)%bifflag==1)then
     call HYMAP3_writeHeader_restart(ftn, n, dimID_bif, bifout_pre_ID, &
          "BIFOUT_PRE", &
          "bifurcation discharge pre", &
          "-", 1, -99999.0, 99999.0)
  endif

  call HYMAP3_closeHeader_restart(ftn)

  ! write state variables into restart file
  if(HYMAP3_routing_struc(n)%useens.eq.0) then
     call HYMAP3_writevar_restart(ftn,n, &
     ...
     !bifurcation outflow previous time step
     if(HYMAP3_routing_struc(n)%bifflag==1)then
        call HYMAP3_writebif_restart(ftn,n, &
             HYMAP3_routing_struc(n)%bifout_pre, &
             bifout_pre_ID)
     endif
  else

     call HYMAP3_writevar_restart_ens(ftn,n,&
          HYMAP3_routing_struc(n)%rivsto, &
          rivsto_ID)
     call HYMAP3_writevar_restart_ens(ftn,n,&
          HYMAP3_routing_struc(n)%fldsto, &
     ...
     !bifurcation outflow previous time step
     if(HYMAP3_routing_struc(n)%bifflag==1)then
        call HYMAP3_writebif_restart_ens(ftn,n, &
             HYMAP3_routing_struc(n)%bifout_pre, &
             bifout_pre_ID)
     endif
  endif

end subroutine HYMAP3_dump_restart

!BOP
! !ROUTINE: HYMAP3_writeGlobalHeader_restart
  ...
  if(LIS_masterproc) then
     ...
     if(HYMAP3_routing_struc(n)%useens.eq.0) then
        call LIS_verify(nf90_def_dim(ftn,'ntiles',&
             ...
             dimID(1)),&
             'nf90_def_dim failed for ntiles in HYMAP3_writeGlobalHeader_restart')

        if(HYMAP3_routing_struc(n)%bifflag==1)then
           call LIS_verify(nf90_def_dim(ftn,'nbif',&
                HYMAP3_routing_struc(n)%nbif,&
                dimID_bif(1)),&
                'nf90_def_dim failed for nbif in HYMAP3_writeGlobalHeader_restart')
        endif
        ...
        if(HYMAP3_routing_struc(n)%bifflag==1)then
           call LIS_verify(nf90_def_dim(ftn,'nbif',&
                HYMAP3_routing_struc(n)%nbif*LIS_rc%nensem(n),&
                dimID_bif(1)),&
                'nf90_def_dim failed for nbif in HYMAP3_writeGlobalHeader_restart')
        endif
     endif
...
#endif
end subroutine HYMAP3_writeGlobalHeader_restart

!BOP
! !ROUTINE: HYMAP3_writeHeader_restart
...
!
! !DESCRIPTION:
!    This routine writes the required NETCDF header for a single HYMAP3
!    variable
!
!   The arguments are:
!   \begin{description}
...
!EOP
  ...
  if(LIS_masterproc) then
     call LIS_verify(nf90_def_var(ftn,trim(standard_name),&
          ...
          'nf90_def_var(1d) failed in HYMAP3_writeHeader_restart')
     ...
     call LIS_verify(nf90_def_var_deflate(ftn,&
          ...
          'nf90_def_var_deflate(1d) failed in HYMAP3_writeHeader_restart')
#endif
...
end subroutine HYMAP3_writeHeader_restart

!BOP
! !ROUTINE: HYMAP3_closeHeader_restart
! \label{HYMAP3_closeHeader_restart}
!
! !INTERFACE:
subroutine HYMAP3_closeHeader_restart(ftn)
...
#endif
end subroutine HYMAP3_closeHeader_restart

!BOP
! !ROUTINE: HYMAP3_writevar_restart
! \label{HYMAP3_writevar_restart}
!
! !INTERFACE:
! Private name: call using LIS_writevar_restart
subroutine HYMAP3_writevar_restart(ftn, n, var, varid)
! !USES:
  use HYMAP3_routingMod
  use LIS_coreMod
  use LIS_logMod
...
!  external :: MPI_GATHERV
  ...
  if(LIS_masterproc) then
     do l=1,LIS_npes
        do i=1,LIS_routing_gdeltas(n,l-1)
           ...
           gtmp(HYMAP3_routing_struc(n)%sindex(ix1,iy1)) = &
  ...
  deallocate(gtmp1)
end subroutine HYMAP3_writevar_restart

!BOP
! !ROUTINE: HYMAP3_writevar_restart_ens
! \label{HYMAP3_writevar_restart_ens}
!
! !INTERFACE:
! Private name: call using LIS_writevar_restart_ens
subroutine HYMAP3_writevar_restart_ens(ftn, n, var, varid)
! !USES:
  use HYMAP3_routingMod
  use LIS_coreMod
  use LIS_logMod
     ...
     do l=1,LIS_npes
        do i=1,LIS_routing_gdeltas(n,l-1)
           do m=1,LIS_rc%nensem(n)
              ...
              gtmp(m + (HYMAP3_routing_struc(n)%sindex(ix1,iy1) -1)* &
                   LIS_rc%nensem(n)) = &
...
end subroutine HYMAP3_writevar_restart_ens

!BOP
! !ROUTINE: HYMAP3_writebif_restart
! \label{HYMAP3_writebif_restart}
!
! !INTERFACE:
! Private name: call using LIS_writebif_restart
subroutine HYMAP3_writebif_restart(ftn, n, var, varid)
! !USES:
  use HYMAP3_routingMod
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  if(LIS_masterproc) then
#if ( defined USE_NETCDF3 || defined USE_NETCDF4 )
     ierr = nf90_put_var(ftn,varid,var,(/1/),&
          (/HYMAP3_routing_struc(n)%nbif/))
     call LIS_verify(ierr,'nf90_put_var failed in LIS_historyMod')
#endif
  endif
end subroutine HYMAP3_writebif_restart
...
! !INTERFACE:
! Private name: call using LIS_writevar_restart_ens
subroutine HYMAP3_writebif_restart_ens(ftn, n, var, varid)
! !USES:
  use HYMAP3_routingMod
  use LIS_coreMod
  use LIS_logMod
