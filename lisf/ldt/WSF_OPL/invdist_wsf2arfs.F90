      ...
      do cc = 1, 1920
         if (total_count(rr,cc) > 0) then
            ...
            if (ocean_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(1, kind(arfs_quality_flag)))
            endif

            ! Bit 1: Precipitation
            if (precip_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(2, kind(arfs_quality_flag)))
            endif

            ! Bit 2: Snow
            if (snow_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(4, kind(arfs_quality_flag)))
            endif

            ! Bit 3: Band 1 (10 GHz) quality
            if (band1_bad_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(8, kind(arfs_quality_flag)))
            endif

            ! Bit 4: Band 2 (18 GHz) quality
            if (band2_bad_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(16, kind(arfs_quality_flag)))
            endif

            ! Bit 5: Band 3 (23 GHz) quality
            if (band3_bad_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(32, kind(arfs_quality_flag)))
            endif

            ! Bit 6: Band 4 (36 GHz) quality
            if (band4_bad_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(64, kind(arfs_quality_flag)))
            endif

            ! Bit 7: Band 5 (89 GHz) quality
            if (band5_bad_count(rr,cc) > total_count(rr,cc)/2) then
               arfs_quality_flag(rr,cc) = IOR(arfs_quality_flag(rr,cc), INT(128, kind(arfs_quality_flag)))
            endif
         else
