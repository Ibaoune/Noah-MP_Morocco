        ...
        do i = 1, nscans
            ...
            if (land_frac_low(j,i) >= 0.0) then
                if (land_frac_low(j,i) < 0.5) then
                    quality_flag(j,i) = IOR(quality_flag(j,i), INT(1, kind(quality_flag)))  ! Ocean
                endif
            endif
            ...
            if (land_frac_low(j,i) > 0.5) then
                ...
                if (tb_18v(j,i) > 0.0 .and. tb_18h(j,i) > 0.0 .and. &
                    ...
                    if (denom > 0.0) then
                        ...
                        if ((sil < 0.005) .and. (tt18 < -2.0)) then
                            precip(j,i) = 1
                            quality_flag(j,i) = IOR(quality_flag(j,i), INT(2, kind(quality_flag)))
                        endif
                    endif
                endif
                ...
                if (tb_36v(j,i) > 0.0 .and. tb_89v(j,i) > 0.0) then
                    if (tb_36v(j,i) - tb_89v(j,i) < -5.0) then
                        snow(j,i) = 1
                        quality_flag(j,i) = IOR(quality_flag(j,i), INT(4, kind(quality_flag)))
                    endif
                endif
            endif
            ...
            if (nband >= 1 .and. band_quality_flags(j,i,1) == 1) then
                quality_flag(j,i) = IOR(quality_flag(j,i), INT(8, kind(quality_flag)))   ! 2^3
            endif

            ! Band 2: 18.7 GHz -> bit 4
            if (nband >= 2 .and. band_quality_flags(j,i,2) == 1) then
                quality_flag(j,i) = IOR(quality_flag(j,i), INT(16, kind(quality_flag)))  ! 2^4
            endif

            ! Band 3: 23.8 GHz -> bit 5
            if (nband >= 3 .and. band_quality_flags(j,i,3) == 1) then
                quality_flag(j,i) = IOR(quality_flag(j,i), INT(32, kind(quality_flag)))  ! 2^5
            endif

            ! Band 4: 36.5 GHz -> bit 6
            if (nband >= 4 .and. band_quality_flags(j,i,4) == 1) then
                quality_flag(j,i) = IOR(quality_flag(j,i), INT(64, kind(quality_flag)))  ! 2^6
            endif

            ! Band 5: 89.0 GHz -> bit 7
            if (nband >= 5 .and. band_quality_flags(j,i,5) == 1) then
                quality_flag(j,i) = IOR(quality_flag(j,i), INT(128, kind(quality_flag))) ! 2^7
            endif
        end do
    end do
