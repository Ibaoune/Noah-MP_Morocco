...
To evaluate the diagnostic sensitivity of drought indicators to SMAP data assimilation, a relative model-derived drought classification was computed based on the OPL-based pooled 2016–2020 reference distributions of Surface Soil Moisture (SSM) and Root-Zone Soil Moisture (RZSM). The analysis compares the temporal evolution of the drought area percentage (pixels classified as D1 or worse, representing a percentile rank ≤ 20%) across the Open Loop (OPL), DA-NoCDF, and DA-CDF experiments.
...
These DA-induced shifts in drought classification suggest that land-surface states are highly sensitive to the observational constraints, particularly in semi-arid domains. However, without a dense, long-term in-situ validation network, these transitions reflect model-derived diagnostic sensitivities and do not imply an actual observed drought alleviation. The differences underscore the critical role that assimilation methodology plays in operational drought monitoring frameworks.
