...
def plot_spatial_impact_fluxes():
    ...
    for i, var in enumerate(variables):
        ...
        ax_r.set_extent([config.DOMAIN_LON_MIN, config.DOMAIN_LON_MAX, config.DOMAIN_LAT_MIN, config.DOMAIN_LAT_MAX], crs=ccrs.PlateCarree())
        ax_r.set_title(f"$\Delta$R: {var} (DA - OL)")
        
        # Delta Anomaly R
        ax_anom = axes[i, 1]
        ax_anom.add_feature(ccrs.cartopy.feature.COASTLINE)
        ax_anom.set_extent([config.DOMAIN_LON_MIN, config.DOMAIN_LON_MAX, config.DOMAIN_LAT_MIN, config.DOMAIN_LAT_MAX], crs=ccrs.PlateCarree())
        ax_anom.set_title(f"$\Delta$Anomaly R: {var} (DA - OL)")

    plt.tight_layout()
    output_path = os.path.join(config.DIR_FIGURES_OPL_VS_DA, "spatial_impact_fluxes.png")
