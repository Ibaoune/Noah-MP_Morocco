...
def load_global_config(global_yaml_path: str) -> Dict[str, Any]:
    ...
    name_main_dir = cfg.get('paths', {}).get('name_main_dir', 'matrix_2016')
    for key, val in cfg.get('paths', {}).items():
        if isinstance(val, str) and '{name_main_dir}' in val:
            cfg['paths'][key] = val.replace('{name_main_dir}', name_main_dir)
...
def load_recipe(recipe_yaml_path: str,
    ...
    name_main_dir = global_cfg.get('paths', {}).get('name_main_dir', 'matrix_2016')
    name_exp = recipe.get('name_exp', 'smap_cdf_sensitivity')
    ...
    for key in ['figure_dir', 'metrics_dir', 'table_dir', 'pdf_report']:
        if key in outputs and outputs[key] is not None:
            rel = outputs[key]
            rel = rel.replace('{name_main_dir}', name_main_dir)
            rel = rel.replace('{name_exp}', name_exp)
