...
class Plotting:
    ...
    def save_figure(fig, out_path, metrics_dict=None):
        ...
        if metrics_dict is not None:
            with open(f"{base_out}_metrics.json", "w") as f:
    ...
    @staticmethod
    def plot_tws_time_series(df, title, subtitle, out_path):
    ...
    @staticmethod
    def plot_tws_mean_panels(grace_da, lis_da, diff_da, title, out_path, bounds, n_matched=None):
    ...
    def plot_alignment_smoke_test_map(lis_mask, esa_mask, common_mask, out_path, bounds, qc_text):
        ...
        lon = lis_mask.coords.get('lon', lis_mask.coords.get('longitude')).values
        lat = lis_mask.coords.get('lat', lis_mask.coords.get('latitude')).values
        
        axes[0].pcolormesh(lon, lat, lis_mask.values, transform=ccrs.PlateCarree(), cmap='Greens', shading='auto', vmin=0, vmax=1)
        axes[0].set_title("(a) LIS Valid Mask", fontsize=13)
        
        axes[1].pcolormesh(lon, lat, esa_mask.values, transform=ccrs.PlateCarree(), cmap='Blues', shading='auto', vmin=0, vmax=1)
        axes[1].set_title("(b) ESA CCI Valid Mask", fontsize=13)
        
        cat_map = np.zeros_like(lis_mask.values, dtype=float)
        cat_map[lis_mask.values > 0] = 1
        cat_map[esa_mask.values > 0] = 2
        cat_map[(lis_mask.values > 0) & (esa_mask.values > 0)] = 3
        cat_map[cat_map == 0] = np.nan
        
        cmap = matplotlib.colors.ListedColormap(['#a1d99b', '#9ecae1', '#756bb1'])
        norm = matplotlib.colors.BoundaryNorm([0.5, 1.5, 2.5, 3.5], cmap.N)
        
        im3 = axes[2].pcolormesh(lon, lat, cat_map, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, shading='auto')
        axes[2].set_title("(c) Overlap Classification", fontsize=13)
        
        for ax in axes:
            Plotting.add_map_features(ax, bounds, margin=0.5)
            
        fig.suptitle("NorthMor spatial alignment and common-data mask", y=1.05)
        
        import matplotlib.patches as mpatches
        legend_elements = [
            mpatches.Patch(color='#a1d99b', label='LIS only'),
            mpatches.Patch(color='#9ecae1', label='ESA only'),
            mpatches.Patch(color='#756bb1', label='Common')
        ]
        axes[2].legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(1.4, 1))
        
        fig.text(0.02, 0.5, qc_text, va='center', bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=10)
        
        Plotting.save_figure(fig, out_path)
        
    @staticmethod
    def plot_spatial_rmse(rmse_da, out_path, bounds, qc_text):
    ...
    @staticmethod
    def plot_spatial_bias(bias_da, out_path, bounds, qc_text):
        fig = plt.figure(figsize=(8, 6.5))
        ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
    ...
    @staticmethod
    def plot_pooled_scatterplots(obs_da, lis_datasets, out_path):
    ...
    @staticmethod
    def plot_domain_time_series(df, out_path):
        fig = plt.figure(figsize=(10, 7))
        gs = gridspec.GridSpec(2, 1, height_ratios=[2, 1], hspace=0.1)
        ax1 = fig.add_subplot(gs[0])
        ax2 = fig.add_subplot(gs[1], sharex=ax1)
    ...
    @staticmethod
    def plot_annual_mean_and_bias(obs_da, lis_da, bias_da, out_path, bounds):
    ...
    @staticmethod
    def plot_data_coverage(valid_count, total_days, out_path, bounds, threshold):
        fig = plt.figure(figsize=(8, 6.5))
        ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
    ...
    @staticmethod
    def plot_map_panels(data_dict, title, out_path, cbar_label, cmap='RdBu', divergent=True):
    ...
    @staticmethod
    def plot_smoke_test_map(lis_valid_mask, common_mask, out_path):
        fig = plt.figure(figsize=(12, 6))
        ax1 = fig.add_subplot(1, 2, 1, projection=ccrs.PlateCarree())
        if lis_valid_mask is not None:
            lon_c = 'lon' if 'lon' in lis_valid_mask.coords else 'longitude'
            lat_c = 'lat' if 'lat' in lis_valid_mask.coords else 'latitude'
            lon = lis_valid_mask.coords[lon_c].values if lon_c in lis_valid_mask.coords else lis_valid_mask.coords.get('east_west').values
            lat = lis_valid_mask.coords[lat_c].values if lat_c in lis_valid_mask.coords else lis_valid_mask.coords.get('north_south').values
            ax1.pcolormesh(lon, lat, lis_valid_mask.values, transform=ccrs.PlateCarree(), cmap='Greens', alpha=0.6, shading='auto')
            dropped = lis_valid_mask.compute().where(lis_valid_mask.compute(), drop=True)
            valid_lats = dropped.coords[lat_c].values if lat_c in dropped.coords else dropped.coords.get('north_south').values
            valid_lons = dropped.coords[lon_c].values if lon_c in dropped.coords else dropped.coords.get('east_west').values
            lat_min, lat_max = np.nanmin(valid_lats), np.nanmax(valid_lats)
            lon_min, lon_max = np.nanmin(valid_lons), np.nanmax(valid_lons)
            ax1.set_extent([lon_min - 0.5, lon_max + 0.5, lat_min - 0.5, lat_max + 0.5], crs=ccrs.PlateCarree())
        ax1.add_feature(cfeature.COASTLINE, linewidth=0.8)
        ax1.add_feature(cfeature.BORDERS, linewidth=0.8, linestyle=':')
        ax1.set_title("LIS Valid Mask (Baseline)")
        ax2 = fig.add_subplot(1, 2, 2, projection=ccrs.PlateCarree())
        lon2 = common_mask.coords.get('lon', common_mask.coords.get('longitude')).values
        lat2 = common_mask.coords.get('lat', common_mask.coords.get('latitude')).values
        ax2.pcolormesh(lon2, lat2, common_mask.values, transform=ccrs.PlateCarree(), cmap='Blues', alpha=0.8, shading='auto')
        dropped2 = common_mask.compute().where(common_mask.compute(), drop=True)
        if dropped2.size > 0:
            valid_lats2 = dropped2.coords.get('lat', dropped2.coords.get('latitude')).values
            valid_lons2 = dropped2.coords.get('lon', dropped2.coords.get('longitude')).values
            lat_min2, lat_max2 = np.nanmin(valid_lats2), np.nanmax(valid_lats2)
            lon_min2, lon_max2 = np.nanmin(valid_lons2), np.nanmax(valid_lons2)
            ax2.set_extent([lon_min2 - 0.5, lon_max2 + 0.5, lat_min2 - 0.5, lat_max2 + 0.5], crs=ccrs.PlateCarree())
        ax2.add_feature(cfeature.COASTLINE, linewidth=0.8)
        ax2.add_feature(cfeature.BORDERS, linewidth=0.8, linestyle=':')
        ax2.set_title("Common Valid Mask (ESA CCI Subset)")
        fig.suptitle("Smoke Test: Spatial Extents", fontsize=14, fontweight='bold')
        Plotting.save_figure(fig, out_path)

    @staticmethod
    def plot_smoke_test_timeseries(obs_ds, lis_datasets, obs_var, common_mask, out_path):
        df_data = {}
        ...
        for exp_name, ds in lis_datasets.items():
            masked_lis = ds["SoilMoist_tavg"].where(common_mask)
            lis_weighted = masked_lis.weighted(weights)
            df_data[exp_name] = lis_weighted.mean(dim=[obs_lat_name, obs_lon_name]).to_series()
        df = pd.DataFrame(df_data)
        df.to_csv(out_path.replace('.png', '.csv'))
        Plotting.plot_time_series(df.dropna(how='all'), "Smoke Test Time Series (Area-Weighted NorthMor)", "Soil Moisture [m³ m⁻³]", out_path)
