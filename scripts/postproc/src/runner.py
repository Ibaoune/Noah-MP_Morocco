...
class RecipeRunner:
    ...
    def _run_diagnostics(self, report: Dict, variables_data: Dict, make_figures: bool, make_hydrology_figures: bool):
        ...
        # --- Runoff Partitioning ---
        if self.recipe.is_diagnostic_enabled('runoff_partitioning') and make_figures:
            logger.info("Dispatching: runoff partitioning diagnostics")
            out_dir = os.path.join(
                self.recipe.outputs.figure_dir, 'runoff_partitioning'
            )
            if not self.dry_run:
                os.makedirs(out_dir, exist_ok=True)
            try:
                from .diagnostics.runoff_partitioning.adapter import run_runoff_partitioning_diagnostics
                files = run_runoff_partitioning_diagnostics(
                    recipe=self.recipe,
                    experiments_catalog=self.experiments_catalog,
                    out_dir=out_dir,
                )
                self._generated_files.extend(files)
                report['diagnostics_run'].append('runoff_partitioning')
            except Exception as e:
                logger.error(f"Error in runoff partitioning diagnostics: {e}")
                report['errors'].append(f"runoff_partitioning: {e}")

        # --- Increment Propagation ---
        logger.info("increment_propagation config=%s, enabled=%s, make_figures=%s", self.recipe.diagnostics.get('increment_propagation'), self.recipe.is_diagnostic_enabled('increment_propagation'), make_figures)
        if self.recipe.is_diagnostic_enabled('increment_propagation') and make_figures:
            logger.info("Dispatching: increment propagation diagnostics")
            out_dir = os.path.join(
                self.recipe.outputs.figure_dir, 'increment_propagation'
            )
            data_out_dir = os.path.join(
                self.global_cfg.get('paths', {}).get('project_root', ''), 'outputs', 'increment_propagation'
            )
            if not self.dry_run:
                os.makedirs(out_dir, exist_ok=True)
                os.makedirs(data_out_dir, exist_ok=True)
            try:
                from .diagnostics.increment_propagation.generator import run_increment_propagation_diagnostics
                files = run_increment_propagation_diagnostics(
                    recipe=self.recipe,
                    experiments_catalog=self.experiments_catalog,
                    global_cfg=self.global_cfg,
                ...
                self._generated_files.extend(files)
                report['diagnostics_run'].append('increment_propagation')
            except Exception as e:
                logger.error(f"Error in increment propagation diagnostics: {e}")
                report['errors'].append(f"increment_propagation: {e}")
