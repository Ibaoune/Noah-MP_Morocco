...
"""
Daisy-Chaining Script for 1Yr_2016 OPL Experiments
"""
...
def main():
    parser = argparse.ArgumentParser(description="Daisy-chaining script for OPL experiments.")
    ...
    base_dir = "/home/mohammad.elaabaribao/lustre/empowermed-ahl6xm8o7mg/users/mohammad.elaabaribao/NoahMP_Morocco/experiments/NorthMor/1Yr_2016/opl"
    os.chdir(base_dir)
    ...
    # If it is the first date, we need the restart file from the end of 2015 
    # (which was produced by ens_opl_2015).
    # ens_opl_2015 copies restarts to ../assim/restarts/surf/ and ../assim/restarts/pert/
    # But wait, OPL also needs the surface restart from 2016-01-01.
    # The spinup directory has LIS_RST_NOAHMP401_201601010000.d01.nc!
    # So for OPL, the initial state can just come straight from spinup restarts!
    expected_start_restart = f"LIS_RST_NOAHMP401_{current_date.strftime('%Y%m%d')}0000.d01.nc"
    
    if current_date == global_start_date:
        spinup_restart = f"/home/mohammad.elaabaribao/lustre/empowermed-ahl6xm8o7mg/users/mohammad.elaabaribao/NoahMP_Morocco/experiments/NorthMor/spinup/restarts/{expected_start_restart}"
        if not os.path.exists(spinup_restart):
            print(f"Error: Initial spinup restart file {spinup_restart} not found!")
            sys.exit(1)
        restart_file_param = spinup_restart
    else:
    ...
    if next_date < global_end_date:
        next_cmd = f"python3 scripts/chain_opl.py --current-date {next_date.strftime('%Y-%m-%d')} --submit"
    else:
...
echo "============================================="
echo " LIS NorthMor - OPL - Period {label}"
